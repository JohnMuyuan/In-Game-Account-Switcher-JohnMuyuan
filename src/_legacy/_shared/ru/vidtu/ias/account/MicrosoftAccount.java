...
public final class MicrosoftAccount implements Account {
    ...
    @NotNull
    public MicrosoftAccount recrypt(@NotNull PasswordCrypt current, @NotNull PasswordCrypt replacement) {
        byte[] decrypted = null;
        try (ByteArrayInputStream byteIn = new ByteArrayInputStream(this.data);
             DataInputStream in = new DataInputStream(byteIn)) {
            String type = in.readUTF();
            if (!current.type().equals(type)) throw new IOException("Unexpected account encryption type: " + type);
            decrypted = current.decrypt(in.readAllBytes());

            try (ByteArrayOutputStream byteOut = new ByteArrayOutputStream();
                 DataOutputStream out = new DataOutputStream(byteOut)) {
                out.writeUTF(replacement.type());
                out.write(replacement.encrypt(decrypted));
                return new MicrosoftAccount(false, this.uuid, this.name, byteOut.toByteArray());
            }
        } catch (Throwable t) {
            throw new RuntimeException("Unable to re-encrypt Microsoft account tokens.", t);
        } finally {
    ...
    public void login(@NotNull LoginHandler handler, Runnable onComplete) {
        try {
            ...
            try (ByteArrayInputStream byteIn = new ByteArrayInputStream(this.data);
                ...
                future = Crypt.readType(in, () -> this.globalPassword(handler));
    ...
    @NotNull
    private CompletableFuture<String> globalPassword(@NotNull LoginHandler handler) {
        String unlocked = GlobalPassword.password();
        if (unlocked != null) return CompletableFuture.completedFuture(unlocked);

        return handler.password().thenApplyAsync(value -> {
